...
struct RSFInfoView: View {
                    ...
                    sectionContainer {
                        DisclosureGroup(isExpanded: $showRSFHours) {
                            VStack(alignment: .leading, spacing: 5) {
                                ...
                                Text("• 11/28: Closed for Thanksgiving Day")
                                Text("• 12/24: Closed for Christmas Eve")
                                Text("• 12/25: Closed for Christmas Day")
                                Text("• 1/1: Closed for New Year's Day")
                            }
                            .padding(.top, 5)
                        } label: {
                            Text("📅 RSF Hours")
    ...
    // MARK: Open in Google Maps Web (if app is not installed)
    func openGoogleMapsWeb() {
        let urlString = "https://www.google.com/maps/dir/?api=1&destination=\(coordinate.latitude),\(coordinate.longitude)"
